    ...
    <div class="row w-75">
        ...
        <div class="col">
            <div></div> <!--class="d-flex flex-column">-->
                ...
                <!--<span class="star-rating" data-rate="3">--><span>{{ str_repeat('★', $review_average) }}</span>
                <span class="review_socre">{{ $review_count }}</span>
                <span style="display:inline;width:50px"> あいうt路え</span>
                <span> 夏季向け</span>
                <p class="">
                    {{$product->description}}
                </p>
        ...
        <h3 class="star-rating" data-rate="3">{{ str_repeat('★', $review_average) }}</h3>
        <span class="review_socre">{{ $review_count }}</span>
        ...
        <div class="row">
          <div class="offset-md-5 col-md-5">
            <form method="POST" action="{{ route('reviews.store') }}">
              ...
              <h4>レビュー内容</h4>
              @error('content')
                 <strong>レビュー内容を投稿してください</strong>
              @enderror
              <textarea name="content" class="form-control m-2"></textarea>
              <input type="hidden" name="product_id" value="{{$product->id}}">
              <button type="hidden" class="btn samuraimart-submit-button ml-2">レビューを追加</button>
            </form>
          </div>
          @endauth
        </div>
